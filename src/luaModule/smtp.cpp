...
// private
bool Smtp::parse(const QByteArray &status) {
    if (status.size() <4) throw sol::error("invalid smtp response");
    const auto code = status.left(3).toInt();
    switch (code) {
        case 220:
        case 235:
        case 250:
